...
export const deleteUser = async (guildId, userId) => {
    try {
      await pool.query(`DELETE FROM "${guildId}" WHERE userId = '${userId}'`);
    } catch (err) {
      return console.error(err);
    }
};
...
export const addUserMessages = async (guildId, userId, amount) => {
  try {
